...
def do_rerun():
    """Safe rerun: try experimental_rerun, otherwise change query params to force rerun."""
    try:
    ...
    try:
        ...
        # last fallback: toggle a session_state key
...
with st.sidebar:
    ...
    st.session_state['user_name'] = st.text_input("Your name / user id", value=st.session_state.get('user_name', ''))
    ...
    with st.form("add_form"):
        ...
        if submitted:
            if not website or str(website).strip() == "":
                ...
                user = st.session_state.get('user_name') or None
                w = add_website(data, user=user)
    ...
    if id_options:
        ...
        with st.form("edit_form"):
            ...
            if submitted_edit:
                ...
                user = st.session_state.get('user_name') or 'web-ui'
                w = update_website(int(selected_id), updates, user=user)
                if w:
                    st.success("Updated")
                    do_rerun()
                else:
